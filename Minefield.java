	...
	public void addInfoCells() {
		...
		for (int i = 0; i<field.length; i++) {
			for (int k = 0; k<field[i].length; k++) {
				...
				/*
				if (field[i][k] instanceof InfoCell) {
					//InfoCell currentCell = new InfoCell(i,k,0);
					//field[i][k] = currentCell;
					int nearbyMines = getNearbyMines(((InfoCell)(field[i][k])));
					field[i][k] = new InfoCell(i,k,nearbyMines);
					//field[i][k] = new InfoCell(i, k, nearbyMines);
				}
				*/
			}
 ...
 * @param row
 * @param col
 * @return returns the Cell at row, col of field
 */
	/*
...
/**
 * 
 * @param status String that represents the status of a cell
 ...
 */
	...
	public int countCellsWithStatus(String status) {
		...
		for (int i = 0; i<field.length; i++) {
			for (int k = 0; k<field[i].length; k++) {
				if (!field[i][k].isMine()) {
					extractedStatus = field[i][k].getStatus();
					if (extractedStatus.equals(status)) {
						count++;
					}
				}
				if (field[i][k].isMine()) {
